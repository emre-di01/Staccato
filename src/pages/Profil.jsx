...
function AvatarBereich({ profil, onUpdate }) {
  ...
  async function hochladen(datei) {
    ...
    if (sErr) { alert('Upload fehlgeschlagen: ' + sErr.message); setLaden(false); return }
  ...
  return (
    <div style={{ display:'flex', flexDirection:'column', alignItems:'center', gap:12 }}>
      ...
      <span style={{ fontSize:12, color:'var(--text-3)' }}>Tippen zum Ändern</span>
...
function ProfilForm({ profil, onUpdate }) {
  ...
  async function speichern() {
    if (!form.voller_name) { setFehler('Name ist erforderlich.'); return }
    setLaden(true)
  ...
  return (
    <div style={s.karte}>
      <h3 style={s.karteTitle}>👤 Persönliche Daten</h3>
      <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:14 }}>
        <div style={{ gridColumn:'span 2', display:'flex', flexDirection:'column', gap:6 }}>
          <label style={s.label}>Voller Name *</label>
      ...
      {erfolg && <p style={s.erfolg}>✅ Gespeichert!</p>}
...
function PasswortForm() {
  ...
  async function aendern() {
    if (form.neu.length < 6) { setFehler('Mindestens 6 Zeichen.'); return }
    if (form.neu !== form.bestaetigung) { setFehler('Passwörter stimmen nicht überein.'); return }
    setLaden(true)
    setFehler('')
  ...
  return (
    <div style={s.karte}>
      <h3 style={s.karteTitle}>🔒 Passwort ändern</h3>
      <div style={{ display:'flex', flexDirection:'column', gap:14 }}>
        <div style={{ display:'flex', flexDirection:'column', gap:6 }}>
          <label style={s.label}>Neues Passwort</label>
          <input type="password" style={s.input} placeholder="Mindestens 6 Zeichen" value={form.neu}
            onChange={e => setForm(f => ({ ...f, neu: e.target.value }))} />
        </div>
        <div style={{ display:'flex', flexDirection:'column', gap:6 }}>
          <label style={s.label}>Passwort bestätigen</label>
          <input type="password" style={s.input} placeholder="Nochmal eingeben" value={form.bestaetigung}
            onChange={e => setForm(f => ({ ...f, bestaetigung: e.target.value }))} />
        </div>
        {fehler && <p style={s.fehler}>{fehler}</p>}
        {erfolg && <p style={s.erfolg}>✅ Passwort geändert!</p>}
        <div style={{ display:'flex', justifyContent:'flex-end' }}>
          <button onClick={aendern} disabled={laden} style={s.btnPri}>
            {laden ? 'Ändere …' : '🔒 Passwort ändern'}
          </button>
        </div>
      </div>
    </div>
...
function AdminPasswortReset() {
  ...
  async function zuruecksetzen() {
    if (!email || !neues) { setFehler('Alle Felder ausfüllen.'); return }
    if (neues.length < 6) { setFehler('Mindestens 6 Zeichen.'); return }
    setLaden(true)
    setFehler('')
  ...
  return (
    <div style={s.karte}>
      <h3 style={s.karteTitle}>🔑 Passwort zurücksetzen (Admin)</h3>
      <p style={{ fontSize:13, color:'var(--text-3)', margin:'0 0 16px' }}>
        Setzt das Passwort eines anderen Nutzers zurück.
      </p>
      <div style={{ display:'flex', flexDirection:'column', gap:14 }}>
        <div style={{ display:'flex', flexDirection:'column', gap:6 }}>
          <label style={s.label}>E-Mail des Nutzers</label>
          <input type="email" style={s.input} placeholder="nutzer@schule.de" value={email}
            onChange={e => setEmail(e.target.value)} />
        </div>
        <div style={{ display:'flex', flexDirection:'column', gap:6 }}>
          <label style={s.label}>Neues Passwort</label>
          <input type="password" style={s.input} placeholder="Mindestens 6 Zeichen" value={neues}
            onChange={e => setNeues(e.target.value)} />
        </div>
        {fehler && <p style={s.fehler}>{fehler}</p>}
        {erfolg && <p style={s.erfolg}>✅ Passwort zurückgesetzt!</p>}
        <div style={{ display:'flex', justifyContent:'flex-end' }}>
          <button onClick={zuruecksetzen} disabled={laden} style={{ ...s.btnPri, background:'var(--warning)' }}>
            {laden ? 'Setze zurück …' : '🔑 Passwort setzen'}
          </button>
        </div>
      </div>
    </div>
...
export default function ProfilSeite() {
  const { profil, rolle, ladeProfil } = useApp()
  const [lokalesProfil, setLokalesProfil] = useState(profil)

  useEffect(() => { setLokalesProfil(profil) }, [profil])

  if (!lokalesProfil) return <div style={{ padding:40, color:'var(--text-3)' }}>Laden …</div>
  ...
  return (
    <div style={{ maxWidth:640, margin:'0 auto' }}>
      <h1 style={s.h1}>⚙️ Mein Profil</h1>
