...
BEGIN
  ...
  DELETE FROM instrumente  WHERE schule_id = p_schule_id; -- lehrer_instrumente: CASCADE
  ...
  -- Auth-User löschen → cascades auf profiles (schule_id NOT NULL, aber Profile werden gelöscht)
  PERFORM delete_auth_user(user_id)
    FROM schul_mitgliedschaften
    WHERE schule_id = p_schule_id;

  -- schul_mitgliedschaften + schul_einladungen: CASCADE
  -- vorstand_*: CASCADE
  -- inventar + inventar_kategorien: CASCADE
  -- demo_anfragen.schule_id: SET NULL
  -- profiles.letzte_schule_id: SET NULL (automatisch)
